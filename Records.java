...
public class Records extends JFrame{
	...
	JLabel freeSpace, username, score, data;
	JButton submit_back;
	public Records(){
		setTitle("Records");
		...
		painel.add(username, new GBC(1,1,1,1).setWeight(1, 1).setAnchor(GBC.EAST).setInsets(0, 10, 0, 0));
		...
		for(int i = 1; i<=10; i++){
			if((users[i] != null)){
				username = new JLabel(users[i]);
				painel.add(username, new GBC(1,1+i,1,1).setWeight(1, 1).setAnchor(GBC.EAST).setInsets(0, 0, 0, 0));
